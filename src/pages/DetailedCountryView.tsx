...
export default function DetailedCountryView({
  selectedCountry,
}: IDetailedCountryView) {
  ...
  const [dataFiltered] = data.countries.filter(
    (country: Details) => country.name === code
  );
      ...
      <div className='detailsContainer container h3 text-light'>
        <div className='row g-4'>
          <h1 className='text-center mb-4'>
            {dataFiltered.name} Country Details
          </h1>
          <ul className='list-group-flush lead'>
            ...
            <li className='list-group-item'>
              <span className='fw-bold'>Emoji:</span> {dataFiltered.emoji}
            </li>
            <li className='list-group-item'>
              <span className='fw-bold'>International code:</span>{' '}
              {dataFiltered.code}
            </li>
            <li className='list-group-item'>
              <span className='fw-bold'>Currency(ies) symbol:</span>{' '}
              {dataFiltered.currency}
            </li>
            <li className='list-group-item'>
              <span className='fw-bold'>Continent:</span>{' '}
              {dataFiltered.continent.name}
            </li>
            <li className='list-group-item'>
              <span className='fw-bold'>Official language(s):</span>{' '}
              {dataFiltered.languages.map(
                (language: ContinentInDetails) => `${language.name} `
              )}
            </li>
            <li className='list-group-item'>
              <span className='fw-bold'>Capital city:</span>{' '}
              {dataFiltered.capital}
            </li>
          </ul>
        </div>
